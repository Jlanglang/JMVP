...
public class CamParaUtil {
	...
	public Size getPropPreviewSize(List<Size> list, float th, int minHeight){
		...
		for(Size s:list){
			if((s.height >= minHeight) && equalRate(s, th)){
				HLog.i(TAG, "PreviewSize:w = " + s.width + "h = " + s.height);
				break;
			}
			i++;
		}
	...
	public Size getPropPictureSize(List<Size> list, float th, int minHeight){
		...
		for(Size s:list){
			if((s.height >= minHeight) && equalRate(s, th)){
				HLog.i(TAG, "PictureSize : w = " + s.width + "h = " + s.height);
				break;
			}
			i++;
		}
	...
	public  void printSupportPreviewSize(Camera.Parameters params){
		...
		for(int i=0; i< previewSizes.size(); i++){
			...
			HLog.i(TAG, "previewSizes:width = "+size.width+" height = "+size.height);
		}
	...
	public  void printSupportPictureSize(Camera.Parameters params){
		...
		for(int i=0; i< pictureSizes.size(); i++){
			...
			HLog.i(TAG, "pictureSizes:width = "+ size.width
					+" height = " + size.height);
		}
	}
	/**��ӡ֧�ֵľ۽�ģʽ
